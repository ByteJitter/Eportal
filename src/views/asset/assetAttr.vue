    ...
    <div class="classify-bg">
      ...
      <v-data-table :headers="headers" :items="desserts" class="elevation-1" no-data-text="暂无数据">
        <template v-slot:items="props">
          ...
          <td class="text-xs-left">{{props.item.asset_category_id.name}}</td>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? '增加资产属性' : '编辑资产属性内容'
    },
    categoryErrors() {
      const errors = []
  ...
  methods: {
    ...
    async getCategories() {
      ...
      if (res.data.status === 200) {
        ...
        data.forEach(cat => {
          catDesserts.push({ _id: cat._id, name: cat.name })
          addCategory.push({ _id: cat._id, name: cat.name })
        })
        this.addCategory = addCategory
        this.catDesserts = catDesserts
        this.catDesserts.unshift({ name: '全部' })
      } else {
        this.$refs.snackbar.showError('刷新重试')
      }
    },
    //获取数据
    async getAttrList(params, search) {
    ...
    saveAdd: Common.throttle(async function() {
      ...
      if (this.editedIndex > -1) {
        ...
        let attrName = this.editedItem.name.trim()
        let res = await this.$request.fetch(
          '/api/asset/updateAttr',
          {
            name: attrName,
            _id: _id
          },
        ...
        if (this.categoryAddSelectId == '' || this.editedItem.name == '') {
          return false
        }
        let categoryId = this.categoryAddSelectId.trim()
        let attrName = this.editedItem.name.trim()
        let res = await this.$request.fetch(
          '/api/asset/addAttribute',
          {
        ...
        if (res.data.status === 200) {
          this.getCategories()
          this.getAttrList(this.currentStatus)
          this.close()
    ...
    handleSearch: Common.throttle(function() {
      ...
      this.currentStatus = paramsSearch
      this.getAttrList(this.currentStatus, 'search')
    }, 2000)
  }
}
</script>
