<template>
  <v-container fluid grid-list-xl>
    <v-layout class="search">
      <v-flex xs12 sm2 md2>
        <v-autocomplete :items="searchData" label="请选择/输入员工姓名" v-model="selectBindId" item-text="name" item-value="_id" no-data-text="暂无数据"></v-autocomplete>
      </v-flex>
      <v-flex xs12 sm2 md2 >
...
export default {
  ...
  data: () => ({
    ...
    isShowDate: false,
    pagination: { sortBy: "employee.name" },
    checkSelected: [],
    searchData: [],
    headers: [
      { text: "员工姓名", align: "center", value: "name", sortable: false},
  ...
  mounted() {
    this.getBonusDeMoment(this.selectBindId,this.expectDate);
  },
  methods: {
    checkClick(item) {
      this.$router.push({
        ...
        query: {
          ...
          auditAndConfirm: "hrEmployeeBonusWait"
        }
      });
    },
    async getBonusDeMoment(name,date,type) {
      ...
      if(date){
        param.choosenMonth = date.substr(0,7)
      }
    ...
    search: Common.throttle(async function() {
      this.getBonusDeMoment(this.selectBindId,this.expectDate,"search");
    }, 2000),
    changeSort(column) {
