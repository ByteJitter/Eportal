<template>
  <div>
    ...
    <v-container fluid grid-list-xl>
      <v-layout wrap align-center>
        <v-flex xs3 class="search-select">
          <v-autocomplete label="请选择/输入公司" :items="searchItems" v-model="searchSelect" no-data-text="暂无数据"></v-autocomplete>
        </v-flex>
        <v-btn color="info" style="margin-left: 20px; border-radius:10px;" @click="searchCompany">
          <v-icon left>search</v-icon>搜索
        </v-btn>
        <v-btn color="info" style="margin-left: 20px; border-radius:10px;" @click="AddCompany">
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout>
                  ...
                  <v-flex xs12 sm6 md4>
                    <v-text-field label="输入公司名称" v-model.trim="addItemData.name" :error-messages="companyErrors" @blur="$v.addItemData.name.$touch()"></v-text-field>
                  </v-flex>
                </v-layout>
              </v-container>
            </v-card-text>
    ...
    <div class="classify-bg">
      ...
      <v-data-table :headers="headers" :items="companyList" class="elevation-1" no-data-text="暂无数据">
        <template v-slot:items="props">
          <td class="text-xs-left" width="600">{{ props.item.name }}</td>
          <td width="260">
            <v-btn color="info" @click="EditCompany(props.item)">编辑</v-btn>
            <v-btn color="info" @click="DeleteCompany(props.item)">删除</v-btn>
          </td>
        </template>
      </v-data-table>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        { text: "公司名称", sortable: false, value: "name" },
  ...
  computed: {
    companyErrors() {
      ...
      !this.$v.addItemData.name.required && errors.push("公司名称为必选字段。");
      return errors;
    }
  },
  mounted() {
  ...
  methods: {
    async getCompanyList(name, type) {
      let param = name && name !== "全部" ? { name } : {};
      let { data } = await this.$request.fetch("/api/admin/companies", param);
      if (data.status === 200) {
        this.companyList = data.result;
        if (!name) {
          this.getSearchItem(this.companyList);
        }
        if (type === "search") {
    ...
    getSearchItem(list) {
      this.searchItems = ["全部"]; //清空数组，因为在编辑成功后调用了getCompanyList（）方法，该数据会重复push
      list.forEach(item => {
    ...
    searchCompany() {
      ...
      if (this.searchSelect.length > 0) {
        this.getCompanyList(this.searchSelect, "search");
      }
    },
    AddCompany() {
      this.dialog = true;
    ...
    addSave: Common.throttle(async function() {
      ...
      let { data } = await this.$request.fetch(
        "/api/admin/companyAdd",
        this.addItemData,
        "post"
      );
      if (data.status === 200) {
        this.$refs.snackbar.showStatus("add");
        this.getCompanyList(this.searchCurrent);
    ...
    async editSave() {
      this.$v.addItemData.$touch();
      if (this.$v.addItemData.name.$invalid) {
        return false;
      }
      let param = {
        company_id: this.addItemData._id,
      ...
      if (data.status === 200) {
        ...
        if (this.isSearch) {
          this.getCompanyList(this.searchItems[this.searchCurrentIndex]);
        } else {
          this.getCompanyList();
        }
      } else if (data.status === 504) {
    ...
    async DeleteDialog() {
      let param = { _id: this.addItemData._id };
      let { data } = await this.$request.fetch(
      ...
      if (data.status === 200) {
        ...
        this.$refs.snackbar.showError("删除失败，该公司下存在机构");
      } else {
